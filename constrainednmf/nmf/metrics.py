...
def r_factor(predict, target):
    """
    Crystallographic convention R-factor for each member of the dataset
    https://dictionary.iucr.org/R_factor
    """
    return torch.mean(
        torch.abs((target - predict)).sum(dim=1) / torch.abs(predict).sum(dim=1)
    )
